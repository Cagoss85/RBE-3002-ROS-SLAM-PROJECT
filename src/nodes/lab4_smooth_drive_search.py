...
class Lab4:
    ...
    def phaseOne(self):
        ...
        if len(centroidResp.centroids) >= 1:
            ...
            centroidResp = centroids()
            rospy.sleep(.375)
            #rospy.loginfo("Checking 1 more time")
    ...
    def phaseTwo(self):
        ...
        mapSaving = subprocess.Popen('rosrun map_server map_saver -f mymap', shell =True)
        rospy.sleep(1)
        mapServierInit = subprocess.Popen('rosrun map_server map_server mymap.yaml', shell =True)
        rospy.sleep(1)
        
        #Robot's Current Position 
        rospy.loginfo('preparing to go home')
        endPose = PoseStamped()
